...
elseif(strcmp(lincirc,'circ'))
    ...
    for i=1:sz(2)
        ...
        ir(:,i) = real(ifft(fft(mymean(:,i)).*fft(invtsp),'symmetric'));    % argument symmetric toevoegen maakt geen verschil!
        % ir(:,i) = fft(mymean(:,i)).*fft(invtsp);
        ir(:,i) = impulseresponse_lengte_juist(ir(:,i),141);
    end
end
