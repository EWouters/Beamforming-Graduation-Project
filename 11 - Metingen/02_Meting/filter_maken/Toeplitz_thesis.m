...
for i=1:2
    ...
    %% plot them
    figure
    plot(s);hold on;plot(s_reproduce(1:2000),'g');plot(x(1:2000),'r');legend('sampled inputsignal','reconstructed inputsignal','filtered inputsignal','Location','SouthEast');
    axis([0 2000 -1 1])
    xlabel('time [ms]', 'FontSize', 10);
    ylabel('amplitude', 'FontSize', 10)
    hold off
    if i==1
        title('Filter of a Delta-peek after 0.5 seconds', 'FontSize', 12)
    else
        title('Filter of a Delta-peek after 10 milliseconds', 'FontSize', 12)
    end
