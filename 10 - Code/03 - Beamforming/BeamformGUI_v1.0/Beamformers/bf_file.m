classdef bf_file < handle
    ...
    %   setOffset       - Set of channels in samples.
    %   getaudiodata       - S
    ...
    methods
        ...
        function locations = setLocations(obj, locations, chanNames)
            % SETLOCATIONS Set Channel Locations.
            %
...
%                 else
        ...
        function obj = addSamples(obj,y, varargin)
            ...
            %
            % NOTE: delayTimes has to be added!
            narginchk(2,5)
            if ~obj.IsInitialized && nargin < 4
                error('bf_file:%s:addSamples: Cannot add samples when object is not initialized and addNew is false',obj.SourceID)
            elseif ~varargin{2}
                error('bf_file:%s:addSamples: Cannot add samples when object is not initialized and addNew is false',obj.SourceID)
            end
        ...
        function [y, totalSamples] = getaudiodata(obj,varargin)
            ...
            if obj.IsInitialized
                ...
                warning('bf_file:%s:getaudiodata: Object is not initialized',obj.SourceID)
                y = [];
            end
            totalSamples = obj.TotalSamples;
