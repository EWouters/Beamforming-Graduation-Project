...
[dum, cindex] = min(mdd);
%  Read in closest mic signal
[yin, fs] = wavread(fName);
yclose = yin(:,cindex(1));
...
%wavwrite(yclose/(sqrt(2)*max(yclose)), fs, 'yClose.wav');
%wavwrite(yDsb/(sqrt(2)*max(yDsb)), fs, 'yDsb.wav');
%wavwrite(yGjbf/(sqrt(2)*max(yGjbf)), fs, 'yGjbf.wav');
%wavwrite(yRobustGsc/(sqrt(2)*max(yRobustGsc)), fs, 'yRobustGsc.wav');
%wavwrite(yMask/(sqrt(2)*max(yMask)), fs, 'yMask.wav');
