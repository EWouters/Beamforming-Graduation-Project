...
[~,fso]= wavread(filename, [1 2]); %just to get the frequency fs
...
tapwin = flattap(nwlen,20);     % This creates a tapering window to reduce edge effects (default 20%)
...
for i=tst:size(det,1)
    ...
    toprocess= sig_grad; %change this to change the signal used in srp
    if fovp.graphicout_flag
