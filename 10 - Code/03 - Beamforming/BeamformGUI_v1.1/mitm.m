...
% Erik: Als mls_sig en samples input argumenten zijn en de offsets
% output kan deze functie ook gebruikt worden voor andere afspeel
% apparaten. Misschien functie in andere file plaatsen. 
%  wanneer streamen gestart wordt kan ergens in beamformer gui dan 
%   offsets = synchronize(sourceSig,recordedSigs) aangeroepen worden.
% dit is nodig om de opgenomen signalen te synchroniseren met de
% afgespeelde signalen. Op de huidige manier kunnen alleen de relatieve
% delays gemeten worden, het is dan nog steeds lastig om bijvoorbeeld
% signal to noise te bepalen.
% alternatief hiervoor is om niet een nieuwe audioplayer te maken, maar in
% gui een play functie aan te roepen, dat is een stuk lastiger.
...
%             disp(['pi: ' num2str(pitch)]);
%             disp(['ro: ' num2str(roll)]);
            ...
            if (index==0)
%                 disp('unable to locate the phone ID in the stored orientation cell!');
            else
