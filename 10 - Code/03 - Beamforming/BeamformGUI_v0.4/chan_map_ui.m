...
%             assignin('base','obj',obj)
        ...
        function pbUpdateChannels_Callback(obj,~,~)
            ...
            switch obj.SelectedSource
                ...
                case 4  % USB
                    ...
                    obj.LocData = [ones(obj.NChan,1), zeros(obj.NChan,5), ones(obj.NChan,1)];
                case 5  % Computer Mic
                    if ~isempty(obj.PanSettings.UI{1}.CompMic.RecObj) %#ok<ALIGN>
                        if obj.PanSettings.UI{1}.CompMic.RecObj.TotalSamples %#ok<ALIGN>
        ...
        function data1 = getData(obj)
            ...
            switch obj.SelectedSource
                ...
                case 2  % File
                    if isempty(obj.PanSettings.UI{1}.File.Data)
                        obj.PanSettings.UI{1}.File.pbFilePath_Callback();
                    end
        ...
        function [data1, totalSamples] = getWindow(obj,source,firstSample,nSamples)
            ...
            switch source
                ...
                case 2  % File
                    ...
                    if firstSample+nSamples <= length(obj.PanSettings.UI{1}.File.Data)
                        data1 = obj.PanSettings.UI{1}.File.Data(firstSample:firstSample+nSamples,:);
                        totalSamples = length(obj.PanSettings.UI{1}.File.Data);
                    end
                case 3  % Simulation
                    if firstSample+nSamples <= length(obj.PanSettings.UI{1}.Simulation.DataOut)
                        data1  = obj.PanSettings.UI{1}.Simulation.DataOut(firstSample:firstSample+nSamples,:);
                        totalSamples = length(obj.PanSettings.UI{1}.Simulation.DataOut);
                    end
                case 4  % USB
                    if firstSample+nSamples <= length(obj.PanSettings.UI{1}.USB.Data)
                        data1  = obj.PanSettings.UI{1}.USB.Data(firstSample:firstSample+nSamples,:);
                        totalSamples = length(obj.PanSettings.UI{1}.USB.Data);
                    end
                case 5  % Computer Mic
                    try data1  = obj.PanSettings.UI{1}.CompMic.RecObj.getaudiodata(); catch; end
                    if firstSample+nSamples <= length(data1)
                        data1  = data1(firstSample:firstSample+nSamples,:);
                        totalSamples = obj.PanSettings.UI{1}.CompMic.RecObj.TotalSamples;
                    end
                otherwise
                    error('Invalid source selection. Can''t get data');
            end
