...
%% 2015-05-21
% This file has been changed by Erik Wouters to accept a matrix input for x
% in order to process mutliple speech column vectors in parallel
...
%% Fix error: 
% Undefined function or variable "jj_o".
% Error in stoi>removeSilentFrames (line 188)
...
for j = 1:length(frames)
    if msk(j)
        ...
        jj_o_end        = jj_o(end);                % Fix error
        count           = count+1;
    end
end

x_sil = x_sil(1:jj_o_end);
y_sil = y_sil(1:jj_o_end);
