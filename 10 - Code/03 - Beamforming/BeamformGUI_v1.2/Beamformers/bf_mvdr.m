        ...
        function obj = Initialize(obj,~,~)
            ...
            if obj.Directivity
                for ii = 1:size(obj.MicNames,2)
                    obj.g(:,ii) = directivity(obj.x_m(ii,:),obj.x_s);
                end
            elseif 0
                for ii = 1:size(obj.MicNames,2)
    ...
    %                 disp(dataOut);
        ...
        function pbProcessAtOnce_Callback(obj,~,~)
            if obj.IsInitialized
                ...
                [ymvdr, W] = mvdrbfBAP(obj.MainObj.DataBuffer.Fs, obj.MainObj.DataBuffer.Fs, obj.L_b, obj.L_be, obj.x_m, obj.x_s, currentWin, obj.epsil, obj.delta_s, 0, obj.g);
                if obj.Directivity
                    name1 = [obj.Name ' Directivity'];
                    fprintf('Processed %i samples using MVDR beamformer with directivity form %i microphones\n',size(ymvdr,1),obj.NMics);
                else
                    name1 = obj.Name;
        ...
        function selectionChanged_Callback(obj,~,~)
            ...
            delta_s = str2double(obj.UI.edDeltaS.String);
            ...
            if isnumeric(speedSound) && ~isempty(speedSound)
                if speedSound <= 100000 && speedSound >= 0;
                    ...
                    obj.UI.edEpsil.String = mat2str(speedSound);
                end
            end
        end
...
function [ymvdr, W] = mvdrbfBAP(fs,c,L_b,L_be,x_m,x_s,xinput,epsil,delta_s,rtflag, g)
    ...
    if ~rtflag
        wait_h = waitbar(0,'Calculating PSDs','Position',[138.4 345.6 308 60],'Name','Please wait');
        for te = 1:sim_lb-1
