...
%   STD_PANEL, STD_SELECTOR_UI, GRID2POS, ...
    ...
    methods
        ...
        function obj = main_window(varargin)
            ...
            % Processors
            obj.BFDSB = bf_dsb(obj.UI.Beamformers.Tabs{2}, obj);
            obj.BFMVDR = bf_mvdr(obj.UI.Beamformers.Tabs{3}, obj);
            obj.SNRProcessor = snr_processor(obj.UI.Intel.Tabs{2},obj.UI.Plots.Tabs{3},obj);
%             obj.IntelProcessor = intel_processor(obj);
...
%             obj.SNRProcessor.Update(); % TODO
        ...
        function UI = graphicsCode(obj)
            ...
            UI.Plots = std_panel(UI.MainFig, grid2pos([2,1,2,2,3,2]),'Plots',...
                {'Audio Window','Audio Signals','Intelligibility','Beampatterns'});
            % add Plots
...
%             UI.pbInitDSB = uicontrol(UI.DSB.Panel,'Style','pushbutton',...
%                 'String','Initialize','Units','Normalized','Callback','obj.MainObj.BFDSB.Initialize()',...
...
%             UI.pbInitMVDRDir = uicontrol(UI.MVDRDir.Panel,'Style','pushbutton',...
%                 'String','Initialize','Units','Normalized','Callback','obj.MainObj.BFMVDRDir.Initialize()',...
%                 'Position',grid2pos([1,2, 1,1, 2,2]));
%             UI.MVDRRIR = std_panel(UI.Beamformers.Tabs{5}, grid2pos([]),'MVDR RIR'); % TODO
            
            % Add Intelligibility panel
            UI.Intel = std_panel(UI.Settings.Tabs{4}, grid2pos([]),'Intelligibility',{'Channels','Run'});
            UI.IntelChannels = std_selector_ui(UI.Intel.Tabs{1},obj,'Intelligibility Channels');
%             UI.SNR = [];
%             UI.STOI = [];
%             UI.PESQ = [];
%             UI.pbUpdateSNR = uicontrol(UI.Intel.Tabs{2},'Style','pushbutton',...
%                 'String','Get SNR','Units','Normalized','Callback','obj.MainObj.SNRProcessor.Initialize()',...
%                 'Position',grid2pos([1,2, 1,1, 2,2]));
%             UI.pbUpdateIntel = uicontrol(UI.Intel.Tabs{2},'Style','pushbutton',...
%                 'String','Get Intel','Units','Normalized','Callback','obj.MainObj.IntelProcessor.Initialize()',...
%                 'Position',grid2pos([2,2, 1,1, 2,2]));
