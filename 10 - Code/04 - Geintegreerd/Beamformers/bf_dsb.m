...
%             assignin('base','obj',obj);
        ...
        function pbProcessAtOnce_Callback(obj,~,~)
            if obj.IsInitialized
                ...
                [ydsb, W] = dsb(currentWin, zeros(obj.WindowSize,size(obj.MicNames,2)), obj.MainObj.DataBuffer.Fs, obj.x_s', obj.x_m', obj.MainObj.DataBuffer.SpeedSound, obj.InverseWeighing);
                obj.MainObj.DataBuffer.addSamples(ydsb, obj.Name,1,1);
                fprintf('Processed %i samples using DSB beamformer from %i microphones\n',size(ydsb,1),obj.NMics);
                obj.Weights = W;
                % white noise gain
