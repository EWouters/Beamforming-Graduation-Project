classdef sys_rep_ui < handle
    ...
    properties
        ...
        TablePos = [-0.5,-0.5,1.04,1.04,-0.01];   % Table Position [left bottom width height z]
...
%             assignin('base','obj',obj)
        ...
        function pbUpdate_Callback(obj,~,~)
            ...
            typeStr = {'Phone','Mic','Source','Beamformer','Noise','Other'};
            markerStr = {'bo','rx','g>','cp','rs','mx'};
            phoneNames = [];
            micNames = [];
            sourceNames = [];
            bfNames = [];
            ...
            for ii = 1:length(chanNames)
                ...
                elseif strfind(chanNames{ii},'Beamformer')
                    bfNames = [noiseNames ii];
            ...
            typeNames = {phoneNames micNames sourceNames bfNames noiseNames otherNames};
            phoneNames = chanNames(phoneNames);
            micNames = chanNames(micNames);
            sourceNames = chanNames(sourceNames);
            bfNames = chanNames(bfNames);
            noiseNames = chanNames(noiseNames);
            otherNames = chanNames(otherNames);
            typeInds = [~isempty(phoneNames) ~isempty(micNames) ~isempty(sourceNames) ~isempty(bfNames) ~isempty(noiseNames) ~isempty(otherNames)];
