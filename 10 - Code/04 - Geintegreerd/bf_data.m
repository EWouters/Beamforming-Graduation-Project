classdef bf_data < handle
    ...
    properties (Constant)
        DefaultLoc = [0 0 0 0 0 1]; % [x y z az el up]
    end
...
%             elseif isnumeric(locations) && all(size(locations) ==  [obj.NChan 6])
%                 obj.Locations = locations;
%             elseif isnumeric(locations) && all(size(locations) ==  [obj.NChan 3])
        ...
        function [y, totalSamples] = getAudioData(obj,varargin)
            ...
            if obj.IsInitialized
                ...
                elseif narginNew == 2
                    ...
                    totalSamples = max(min(obj.CurrentSample(obj.names2inds(varargin{1}))-1),0);
                elseif narginNew == 3
...
%             if length(delaySamples) == 1 %|| all(chanInds == 1:obj.NChan)
                y = [y(delaySamples:end,:); ...
                     zeros(delaySamples-1,length(chanInds),obj.audioDataType)];
%             else
...
%             obj.setTotalSamples(obj.TotalSamples - max(delaySamples));
            % Set current samples
